...
class APATrainer(BaseTrainer):
    """
    The APATrainer uses Advantage-Induced Policy Alignment to optimise language models.

    Attributes:
        **config** (`APAConfig`) -- Configuration object for PPOTrainer. Check the documentation of `APAConfig` for more
            details.
    ...
    def __init__(
        ...
        """
        Initialize APATrainer.

        Args:
            config (`APAConfig`):
                Configuration object for PPOTrainer. Check the documentation of `APAConfig` for more details.
        ...
        if not isinstance(config, APAConfig):
            raise ValueError(f"config must be a APAConfig, got {type(config)}")
        ...
        self.accelerator.init_trackers(
            ...
            config=dict(trl_apa_trainer_config=config.to_dict()) if not is_using_tensorboard else config.to_dict(),
        ...
        if self.dataset is not None:
            ...
            warnings.warn(
                ...
                " prepare your dataloader yourself with `dataloader = apa_trainer.accelerator.prepare(dataloader)`"
                " and using `torch.utils.data.DataLoader`, or pass a dataset to the `APATrainer`. Please "
    ...
    def step(
        ...
        timing["time/apa/forward_pass"] = time.time() - t
        ...
        with torch.no_grad():
            ...
            timing["time/apa/compute_rewards"] = time.time() - t

            t = time.time()
            values, advantages, returns = self.compute_advantages(values, rewards, masks)
            timing["time/apa/compute_advantages"] = time.time() - t
        ...
        timing["time/apa/optimize_step"] = time.time() - t
        ...
        timing["time/apa/calc_stats"] = time.time() - t
        stats["apa/learning_rate"] = self.optimizer.param_groups[0]["lr"]
        ...
        # Log the total apa time
        timing["time/apa/total"] = time.time() - t0
        stats.update(timing)
    ...
    def compute_rewards(
        ...
        for score, logprob, ref_logprob, mask in zip(scores, logprobs, ref_logprobs, masks):
            ...
            non_score_reward = 0 * kl
            # In APA training stage, KL penalty is not added to the reward
            # so in APA training stage, non_score_reward is always 0
    ...
    def loss(
        ...
        return sq_loss, self.config.vf_coef * vf_loss, flatten_dict(stats)
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        stats = {
            ...
            "apa/mean_non_score_reward": mean_non_score_reward,
            "apa/mean_scores": mean_scores,
            "apa/std_scores": std_scores,
        }
        ...
        for k, v in data["train_stats"].items():
            stats[f"apa/{k}"] = torch.mean(v, axis=0)
        stats["apa/val/var_explained"] = 1 - stats["apa/val/error"] / stats["apa/returns/var"]
        return stats
